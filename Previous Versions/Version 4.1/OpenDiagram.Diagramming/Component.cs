        ...
        public Bitmap GetResource(string name, string assembly)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    PropertyInfo[] allProps = type.GetProperties(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
                    int j = allProps.Length;
                    bitmap = null;
                    for (int i = allProps.Length - 1; i >= 0; --i)
                    {
                        PropertyInfo p = allProps[i];
                        if (type.FullName + "." + p.Name == name)
                        {
                            var bmp = allProps[i].GetValue(null, null) as Bitmap;
                            if (bmp != null)
                            {
