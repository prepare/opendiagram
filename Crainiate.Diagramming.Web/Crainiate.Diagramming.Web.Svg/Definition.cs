...
namespace Crainiate.Diagramming.Web.Svg
{
	...
	{
		//Property variables
		private GraphicsPath _path; //Cloned path to use
		...
		public void Rotate(float degrees)
		{
			...
			RectangleF rectF = _path.GetBounds();
		...
		public GraphicsPath Path
		{
			...
			set
			{
				if (value != null)
				{
					...
					_path = (GraphicsPath) value.Clone();
					rectF = _path.GetBounds();
		...
		private string ExtractRectangleImplementation(GraphicsPath path)
		{
			...
			if (! mIsRectangle)
			{
				if (! IsPathRectangleImplementation(_path)) throw new DefinitionException("The path cannot be converted to an SVG rectangle");
			}
		...
		private string ExtractEllipseImplementation(PathData pathData)
		{
			...
			if (! mIsEllipse)
			{
				if (! IsPathEllipseImplementation(_path)) throw new Exception("The path cannot be converted to an SVG ellipse");
			}
		...
		private string ExtractPolygonImplementation(PathData pathData)
		{
			...
			if (! mIsPolygon)
			{
				if (! IsPathPolygonImplementation(_path)) throw new Exception("The path cannot be converted to an SVG polygon");
			}
